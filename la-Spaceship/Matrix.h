...
/* Default constructor */
MTXTMP Matrix<T, rows, cols>::Matrix() : myVal({}) {
...
MTXTMP void Matrix<T, rows, cols>::SetRow(unsigned int row, const std::array<T, cols>& data)
{
	myVal[row] = data;
}
...
/* is upper triangular? */
MTXTMP bool Matrix<T, rows, cols>::IsUpperTriangular() const {
	return calc_uptriag<T, rows, cols>()(*this);
}

/* is lower triangular? */
MTXTMP bool Matrix<T, rows, cols>::IsLowerTriangular() const {
	return calc_lwtriag<T, rows, cols>()(*this);
}

/* is triangular? */
MTXTMP bool Matrix<T, rows, cols>::IsTriangular() const {
	return IsUpperTriangular() || IsLowerTriangular();
}

/* is diagonal? */
MTXTMP bool Matrix<T, rows, cols>::IsDiagonal() const {
	return IsUpperTriangular() && IsLowerTriangular();
}

MTXTMP std::ostream & operator<<(std::ostream & cout, const Matrix<T, rows, cols>& matrix)
{
	for (unsigned int i = 0; i < cols; ++i)
	{
